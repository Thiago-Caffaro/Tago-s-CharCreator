...
export function AddCardMenu({ onAdd }: Props) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute top-full left-0 mt-2 w-52 bg-[#1e1e1e] border border-[#333] rounded-xl shadow-xl z-10 py-1 overflow-hidden max-h-80 overflow-y-auto">
          {types.map(t => (
            <button
              key={t.slug}
              className="w-full flex items-center gap-3 px-3 py-2 text-xs text-gray-300 hover:bg-[#242424] transition-colors"
              onClick={() => { onAdd(t.slug, t.label); setOpen(false) }}
            >
              <ContextCardTypeIcon type={t.slug} size={13} />
